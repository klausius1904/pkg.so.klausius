...
char **process_filestats(struct stat* file_stats, char**file_names, int* file_descriptors, int size){
  ...
  for(int i = 0; i<size ; i++){
    messages[i] = process_filestat(file_stats[i],file_descriptors[i], file_names[i]);
  }
  messages[size] = NULL;
  return messages;
}
...
char *process_filestat(struct stat file_stat,int file_descriptor, char *file_name){
  ...
  if(S_ISREG(file_stat.st_mode)){
    if(strcmp(".bmp", file_name+(strlen(file_name)-4)) == 0){
      printf("%s\n", make_bmp_message(file_stat, file_descriptor, file_name));
    }
    else{
      printf("%s\n", make_fil_message(file_stat, file_descriptor, file_name));
    }
  }
      ...
      printf("%s\n", make_link_message(file_stat, file_descriptor, file_name));
  }
  else {
    printf("nu e ok");
  }
